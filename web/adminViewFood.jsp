<%-- 
    Document   : adminViewFood
    Created on : Jan 16, 2021, 5:09:52 PM
    Author     : Admin
--%>
...
<html>
    ...
    <body>
		...
		<h2>Foods</h2>
		<form action="MainController?page=food" method="POST" onsubmit="return checkRemove()">
				...
				<tbody>
					<c:forEach items="${requestScope.result}" var="productDTO" varStatus="counter">
						<tr>
							...
							<td>
								<a href="MainController?page=food&action=updateProduct&txtProductID=${productDTO.productID}&txtProductName=${productDTO.productName}&txtPicture=${productDTO.picture}&txtDescription=${productDTO.description}&txtPrice=${productDTO.price}&txtQuantity=${productDTO.quantity}&index=${requestScope.index}">Edit</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
		...
		<div class="paging">
			<c:forEach begin="1" end="${endPage}" var="i">
				<a href="MainController?action=&index=${i}">${i}</a>
			</c:forEach>
		</div>
