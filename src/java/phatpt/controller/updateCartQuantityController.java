...
public class updateCartQuantityController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String[] quantity = request.getParameterValues("txtCartQuan");
        HttpSession session = request.getSession();
        String totalPrice = String.valueOf(session.getAttribute("totalPrice"));
        try {
            productCart cartObj = (productCart) session.getAttribute("shoppingCart");
            int i = 0;
            float total = 0;
            for (productDTO dto : cartObj.getCart().values()) {
                dto.setQuantity(Integer.parseInt(quantity[i++]));
                dto.setCartPrice(dto.getPrice() * dto.getQuantity());
                total += dto.getCartPrice();
            }
            session.setAttribute("totalPrice", total);
        } catch (Exception e) {
            log("ERROR AT updateCartQuantityController: " + e.getMessage());
        } finally {
