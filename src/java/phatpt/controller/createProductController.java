...
public class createProductController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String productPic = request.getParameter("txtProductPic");
			float price = Float.parseFloat(request.getParameter("txtProductPrice"));
			int quantity = Integer.parseInt(request.getParameter("txtQuantity"));
			Date createDate = new Date();
			productDAO dao = new productDAO();
			dao.insert(productID, cateID, productName, description, productPic, price, createDate, quantity);		
		} catch (Exception e) {
			log("ERROR AT createFood: " + e.getMessage());
		} finally {
			response.sendRedirect("loadFoodController");
		}
	}
